...
def generate_figures(mode="evaluation", name="", load_weights=("", "")):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        if mode == "evaluation":
            corr_avg = compute_corr_score(sp_temp, sp, tv_dim)

            # write a .txt with results and model params
            lines = ['Corr_Average:' + str(corr_avg)]
...
out = "figs/NEW_with_only_1_loss_for_speech_nasal_only_TVs_" + str(date.today()) + "_H_" + str(datetime.datetime.now().hour)
out_dir = "./figs/NEW_with_only_1_loss_for_speech_nasal_only_TVs_" + str(date.today()) + "_H_" + str(datetime.datetime.now().hour) + "/"
...
parser.add_argument('--train_data', type=str,  default=base_dir+"Nasal_glottal_SI/train_audio_nasal_only_2sec.data",
                    help='path to training data')

parser.add_argument('--test_data', type=str,  default=base_dir+"Nasal_glottal_SI/test_audio_nasal_only_2sec.data",   #female_voice.data",
                    help='path to testing data')

parser.add_argument('--dev_data', type=str,  default=base_dir+"Nasal_glottal_SI/dev_audio_nasal_only_2sec.data",
...
start.record()
#
trainTogether_newTechnique(epochs=5, init=True, train_E=True, loader_eval="train", save_better_model=True, name='init')
