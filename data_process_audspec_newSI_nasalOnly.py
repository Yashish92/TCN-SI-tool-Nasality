...
def generate_data(path_audio, path_params, audio_time=2, sampling_rate=16000, random=False):
    ...
    # directory = path_audio[:-1] + "_nasal_glottal_2sec" + ".data"  # save data to this path
    directory = path_audio[:-1] + "_nasal_only_2sec_ext" + ".data"  # save data to this path
        ...
        for files in tqdm(filenames):
            if files[-5] != ')' and files[-10] != ')':
                ...
                if TV_data.shape[0] < tv_len:
                    ...
                    TV_data = np.expand_dims(TV_data, axis=1)
                    ...
                    for i in range(0, no_segs):
                        if first_test:
                            ...
                            tv_data_seg = np.pad(TV_data[(tv_len * i):, :], ((0, pad_amt), (0, 0)), 'constant',
                                                 constant_values=(0, 0))
                            tv_data_seg = np.expand_dims(tv_data_seg, axis=0)
